...
k_nnrti=sr_nnrti[,nnrti_pos]
k_nnrti=as.data.frame(k_nnrti)
...
#Time
time_bt = x_nnrti[,"Time_to_fail"] #time backtransformed (real time)
time_bt[(which(is.na(time_bt)))[1]] = 4.1 * 12 #ART duration is 4.1 years for people with VL>80, assumption duration same for people with VL>1000, see Labhart
...
y_mean = apply(y_nnrti_bt,2,mean) #mean by NRTI drugs
y_sd = apply(y_nnrti_bt,2,sd) #sd by NRTI drugs
y_nnrti = (y_nnrti_bt - structure(rep(y_mean,each=dim(y_nnrti_bt)[1]),dim=dim(y_nnrti_bt)))/ #transformed NRTI drugs
  structure(rep(y_sd,each=dim(y_nnrti_bt)[1]),dim=dim(y_nnrti_bt))
...
mod_NNRTI<-stan_model(file="C:/Users/ahauser/Desktop/systematic_review_AH2/R/stan/TAM/NNRTI_stan.stan")
...
save(fit_NNRTI,file="C:/Users/ahauser/Desktop/systematic_review_AH2/R/stan/TAM/fit_NNRTI.RData")
...
#Figure
load("C:/Users/ahauser/Desktop/systematic_review_AH2/R/stan/TAM/fit_NNRTI.RData")
d<-summary(fit_NNRTI)$summary
