...
sr=sr[,-c(which(colnames(sr) %in% c("inclusion","PID","RT_seq","PR_seq","Seq_Tech")),
          which(colnames(sr)=="on_1stline"):which(colnames(sr)== "subtyp_c"))]
#first
...
s<-c(which(colnames(sr)=="nb_on_treatment"):which(colnames(sr)=="TAM"))
...
#statistics (we do it for nrti data, as nnrti data in included in nrti - i.e. all studies removed in NRTI appeared to be also removed in NNRTI)
#genotyped
...
which_na<-which_na_nnrti | which_na_nrti
sr[which_na_nnrti,1]
sr[which_na_nrti,1]
sum(!which_na_nrti)
sum(!which_na_nnrti)
...
#we use the nrti data in statistics, as nnrti data in included in nrti - i.e. all studies removed in NRTI appeared to be also removed in NNRTI
...
sum(sr[!which_na_nrti | !which_na_nnrti,]$nb_vl_failed)
median(sr[!which_na_nrti | !which_na_nnrti,]$nb_vl_failed)
min(sr[!which_na_nrti | !which_na_nnrti,]$nb_vl_failed)
max(sr[!which_na_nrti | !which_na_nnrti,]$nb_vl_failed)

median(sr[!which_na_nrti | !which_na_nnrti,]$Genotyped/sr[!which_na_nrti | !which_na_nnrti,]$nb_vl_failed)
min(sr[!which_na_nrti | !which_na_nnrti,]$Genotyped/sr[!which_na_nrti | !which_na_nnrti,]$nb_vl_failed)
max(sr[!which_na_nrti | !which_na_nnrti,]$Genotyped/sr[!which_na_nrti | !which_na_nnrti,]$nb_vl_failed)
sr[!which_na_nrti | !which_na_nnrti,]$Publication

median(sr[!which_na_nrti | !which_na_nnrti,]$enroll_end,na.rm = TRUE)
min(sr[!which_na_nrti | !which_na_nnrti,]$enroll_end,na.rm = TRUE)
max(sr[!which_na_nrti | !which_na_nnrti,]$enroll_end,na.rm = TRUE)
...
#Publication
sort(unique(sr[!which_na_nrti | !which_na_nnrti,]$Publication))
...
print(sr_nrti$Publication)

#covariates: "backtransformed" (real) data (time in months, drug in proportion)
x_nrti[,-grep("Time_to_fail",colnames(x_nrti))]=x_nrti[,-grep("Time_to_fail",colnames(x_nrti))]/100
x_nrti_bt=x_nrti

#k, n, N and M
n_nrti=sr_nrti$Genotyped
k_nrti=as.data.frame(sr_nrti[,nrti_pos])
k_nrti[is.na(k_nrti)]=0 #NA is considered as 0
N_nrti=length(n_nrti)
M_nrti=dim(k_nrti)[2]
study_nrti=sr_nrti$Publication

save(n_nrti,k_nrti,N_nrti,M_nrti,x_nrti_bt,sr_nrti,file="results/data_NRTI.RData")
#-------------------------------------------------------------------------------------------------------------
...
#Time
time_bt = x_nrti_bt[,"Time_to_fail"] #time backtransformed (real time)
time_bt[sr_nrti$Publication=="Labhardt 2016"]= 4.1 *12
na_time_pos = which(is.na(time_bt)) #position of NA values
nna_time_pos = which(!is.na(time_bt)) #ART duration is 4.1 years for people with VL>80, assumption duration same for people with VL>1000, see Labhart
sr_nrti[na_time_pos,"Publication"]
...
#mod_NRTI<-stan_model(file="../stan_models/analysis/NRTI_stan3.stan") #does not calculate p_x_study
mod_NRTI<-stan_model(file="../stan_models/analysis/NRTI_stan2.stan") #calculate p_x_study
...
save(B_list_assumed_NRTI,fit_NRTI,file="results/fit_NRTI.RData")
...
save(fit_TAM,file="results/fit_TAM.RData")
###############################################################################################################################################################################
###############################################################################################################################################################################
#Figure
load(file="results/fit_TAM.RData")
d4<-summary(fit_TAM)$summary
load("results/fit_NRTI.RData")
d<-summary(fit_NRTI)$summary
