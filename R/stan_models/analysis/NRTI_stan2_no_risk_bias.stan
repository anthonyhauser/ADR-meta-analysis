...
data {
  ...
  vector[N-2] time_tr_nna; //ART time in non-missing position
  real mean_t; //mean of ART time
  real var_t; //variance of ART time
  int na_time_pos[2]; //position of missing ART times
  int nna_time_pos[N-2]; //position of non-missing ART times
...
model {
  ...
  time_imput[2] ~ gamma(mean_t^2/var_t,mean_t/var_t);
